...
const ScenarioChallenge = () => {
  ...
  useEffect(() => {
    const fetchScenarios = async () => {
      try {
        ...
        const response = await fetch(`${config.API_URL}/scenarios?domain=${selectedDomain}`, {
          headers: {
  ...
  const handleResetScenarios = async () => {
    try {
      ...
      const response = await fetch(`${config.API_URL}/scenarios/reset`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleDomainSelect = async (domainId) => {
    try {
      ...
      const response = await fetch(`${config.API_URL}/scenarios?domain=${domainId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (data && data.length > 0) {
        ...
        const generateResponse = await fetch(`${config.API_URL}/scenarios/generate`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const handleRefreshDomain = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch(`${config.API_URL}/scenarios/refresh/${selectedDomain}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleGenerateMore = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch(`${config.API_URL}/scenarios/generate`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
